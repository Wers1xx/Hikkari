...
## 🪐 Hikkari 2.1.0
 ...
 - add -w and --wipe argument for python3 -m hikkari for wipe session, databases, loaded modules
 ...
 - migrate inline bot from aiogram to HikkariTL
 ...
 - fix(hikkari_config): simplify formatting of option lines
...
- remove "hikkari" from bot username
- fix asset channel
...
- remove invokecmd
- remove uninstall_hikkaricmd
- remove debugmodcmd
- add new validator RandomLink
- replace validators in help, hikkariinfo, tester to RandomLink
...
- loging improvements
- add invert_media in hikkari-tl
- fix zero CPU %
- added config split
...
- Telethon has been completely updated, moving to hikkari-tl
- update hikkari-tl to 1.1.0
- added prefix to ping
- removed official support serv00, tothost, goorm, railway, termux
- rename folder "hikka" to "hikkari"
- fix close btn
- fix native imports
- added converting Hikka backups to Hikkari
...
- added config for show "Hikkari" in info
- removed UnitHeta
...
- https://github.com/Wers1xx/Hikkari/issues/115
- https://github.com/Wers1xx/Hikkari/issues/114
- https://github.com/Wers1xx/Hikkari/issues/119
- https://github.com/Wers1xx/Hikkari/issues/120
- Other changes
...
- Drop Okteto support
- Show Hikkari platform and version in Telegram sessions list
...
- Completely drop Hikkari support due to legacy code, limits and removing of free tier
...
- Publish hikka telethon fork and migrate to it in requirements, thereby fixing the deployment error on Hikkari
- Add custom emojis filter to `utils.remove_html`
- Fix `client.get_perms_cached`
- Fix translation flaw in `HikkariSecurity`
...
- Add platform-specific Hikkari emojis to premium users
...
- Add support for multiaccounting on Hikkari
...
- Remove `hikkari3` from classic requirements, along with hikkari installation code snippet
...
- Update quickstart
- New fields in HikkariInfo
- Remove suffix `Hikkari` if user specified it somewhere below
...
- Fix hikkari-specific config error
...
- Add automatic webpage bot unblock in hikkari waker
- Add secure boot feature
- Update native hikkari postgre database saving method
...
- Add `banner_url` config var to `HikkariInfo`
- Add `loader.validators.Hidden`
- Add `websockets` dependency, so users can load hikarichat on Hikkari
...
- Add `client.force_get_entity` to bypass Hikkari Cacher
...
- Fix translation issue in HikkariConfig
- Fix `.dump`
...
- Add additional Hikkari deps
- Fix `load_module` reattempt
- Reorder database read-writes to make postgres the ladder
- Make redis optional for non-hikkari users
- Add source to Pipfile
- New Hikkari dependencies list in Pipfile
...
- Add banner with Hikkari installation status
- Reorder config saving in web
- Fix `Unauthorized` error on Hikkari
...
- Update gitignore so git doesn't count shit on hikkari
- Visual hikkari fixes in updater
- Deepsource fixes
- Add `utils.get_entity_url`, `utils.get_message_link`, `utils.remove_html`, `utils.get_kwargs`
- Disable modules debugging on hikkari
...
- Refactor `hikkari.py` app searching algorithm
- Refactor postgresql database saving process
- Fix hikkari restart message not being edited
- Add hikkari waker
- Make `hikka-` app naming optional

## 🌑 Hikkari 1.2.0

- Add full-featured Hikkari support with additional buildpacks
...
- Drop Uniborg and Raphielgang compatibility layer. More info in [2d525ab](https://github.com/hikariatama/Hikkari/commit/2d525ab6b0e6b9d7ccd7408bbe175cea24d780a5)
- Minor bug fixes

## 🌑 Hikkari 1.1.0

- Make inline commands' docstrings translatable
- Introducing HikkariDynamicTranslate - the possibility for developers to translate their own modules to other language without usage of native translates
...
- Refactor HikkariSecurity, especially the bounding mask control
...
- Add ability to disable HikkariDL natively
- Fix Updater so it works if you have troubles with inline mode

## 🌑 Hikkari 1.0.22

Add HikkariDL module to use inline download buttons in verified channels
