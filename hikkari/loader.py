...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _hikkari_client_id_logging_tag = copy.copy(  # noqa: F841
...
class Modules:
    ...
    def __init__(
        ...
        self.client.hikkari_inline = self.inline
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.hikkari_commands)
                inline_handlers.update(module.hikkari_inline_handlers)
                callback_handlers.update(module.hikkari_callback_handlers)
                watchers.extend(module.hikkari_watchers.values())
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.hikkari_commands))
            )

        for _command, cmd in instance.hikkari_commands.items():
    ...
    def register_inline_stuff(self, instance: Module):
        ...
        for name, func in instance.hikkari_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.hikkari_watchers.values():
