...
class InlineManager(
    ...
    :type client: hikkari.tl_cache.CustomTelegramClient
    :type db: hikkari.database.Database
    :type allmodules: hikkari.loader.Modules
    """
    ...
    async def register_manager(
        ...
        self._bot_client = TelegramClient(
            SQLiteSession(
                os.path.join(main.SESSIONS_DIR, f"hikkari-{self._me}-bot-{bot_uid}")
            ),
            self._client.api_id,
        ...
        for folder in _folders.filters:
            if getattr(folder, "title", None) == "Hikkari":
                if any(
                    [
        ...
        try:
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start hikkari init"
                )
            except Exception:
