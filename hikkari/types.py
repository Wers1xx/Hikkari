...
class Module:
    ...
    @property
    def hikkari_commands(self) -> dict[str, Command]:
        """List of commands that module supports"""
        return get_commands(self)
    ...
    @property
    def hikkari_inline_handlers(self) -> dict[str, Command]:
        """List of inline handlers that module supports"""
        return get_inline_handlers(self)
    ...
    @property
    def hikkari_callback_handlers(self) -> dict[str, Command]:
        """List of callback handlers that module supports"""
        return get_callback_handlers(self)
    ...
    @property
    def hikkari_watchers(self) -> dict[str, Command]:
        """List of watchers that module supports"""
        return get_watchers(self)
    ...
    @hikkari_commands.setter
    def hikkari_commands(self, _):
        pass
    ...
    @hikkari_inline_handlers.setter
    def hikkari_inline_handlers(self, _):
        pass
    ...
    @hikkari_callback_handlers.setter
    def hikkari_callback_handlers(self, _):
        pass
    ...
    @hikkari_watchers.setter
    def hikkari_watchers(self, _):
        pass
    ...
    async def _decline(
        ...
        self._db.set(
            "hikkari.main",
            "declined_joins",
            list(set(self._db.get("hikkari.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
    ...
    async def request_join(
        ...
        match channel:
            ...
            case _ if channel.id in self._db.get("hikkari.main", "declined_joins", []):
                if assure_joined:
                    raise LoadError(
        ...
        self.hikkari_wait_channel_approve = (
            self.__class__.__name__,
            channel,
            reason,
        )
        ...
        with contextlib.suppress(AttributeError):
            delattr(self, "hikkari_wait_channel_approve")
    ...
    async def import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?hikkari_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?hikkari_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Hikkari version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"hikkari.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("hikkari.main", "stats", True)
            and url is not None
