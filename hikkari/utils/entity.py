...
async def asset_forum_topic(
    ...
    async def create_topic() -> ForumTopic:
        result = await client(
            CreateForumTopicRequest(
                peer=entity,
                title=title,
                icon_emoji_id=(icon_emoji_id if client.hikkari_me.premium else None),
            )
        )
    ...
    forums_cache = db.pointer("hikkari.forums", "forums_cache", {})
...
async def get_topic_id(db: "Database", topic_name: str) -> int | None:
    ...
    """
    try:
        forums_cache = db.get("hikkari.forums", "forums_cache", {})
        return forums_cache.get("hikkari-userbot", {}).get(topic_name)
    except Exception:
        return None
