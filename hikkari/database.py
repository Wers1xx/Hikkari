...
class Database(dict):
    ...
    async def ensure_content_channel(self):
        ...
        existing_channel_id = self.get("hikkari.forums", "channel_id", None)
        ...
        if existing_channel_id:
            try:
                ...
                content_channel = None
                self.set("hikkari.forums", "forums_cache", {"hikkari-userbot": {}})
        ...
        if not content_channel:
            async for dialog in self._client.iter_dialogs():
                if dialog.title and "hikkari-userbot" in dialog.title.lower():
                    ...
                    self.set("hikkari.forums", "channel_id", int(dialog.entity.id))
                    break
    ...
    def read(self):
        ...
        try:
            ...
            if re.search(r'"(hikka\.)(\S+\":)', db):
                ...
                db = re.sub(r"(hikka\.)(\S+\":)", lambda m: "hikkari." + m.group(2), db)
    ...
    async def store_asset(self, message: Message) -> int:
        ...
        try:
            _assets_topic_id = self.get("hikkari.forums", "forums_cache", {})[
                "hikkari-userbot"
            ]["Assets"]
        except (TypeError, KeyError):
            raise NoAssetsChannel("Tried to save asset to non-existing asset topic.")

        if not (_content_channel_id := self.get("hikkari.forums", "channel_id", None)):
