...
class Hikkari:
    ...
    def _migrate_sessions(self):
        ...
        with os.scandir(BASE_DIR) as entries:
            legacy = [
                ...
                if entry.is_file()
                and entry.name.startswith("hikkari-")
    ...
    async def save_client_session(
        ...
        *,
        ...
        client.hikkari_db = database.Database(client)
        await client.hikkari_db.init()

        try:
            db = client.hikkari_db
            existing = db.get("hikkari.inline", "custom_bot", False)
        except Exception:
            existing = False
        ...
        if (
            ...
            while bot := input(
                "You can enter a custom bot username or leave it empty and Hikkari will generate a random one: "
            ):
                bot = bot.strip()
                bot = bot.lstrip("@")
                if any(
                ...
                try:
                    if await self._check_bot(client, bot):
                        db.set("hikkari.inline", "custom_bot", bot)
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            logging.debug(
                ...
                client.tg_id,
                client.hikkari_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
