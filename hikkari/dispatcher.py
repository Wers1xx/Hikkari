...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.hikkari_me.id
        self._cached_usernames = set()

        if self._client.hikkari_me.username:
            self._cached_usernames.add(self._client.hikkari_me.username.lower())

        if self._client.hikkari_me.usernames:
            self._cached_usernames.update(
                u.username.lower()
                for u in getattr(self._client.hikkari_me, "usernames", [])
            )

        self._cached_usernames.add(str(self._client.hikkari_me.id))
